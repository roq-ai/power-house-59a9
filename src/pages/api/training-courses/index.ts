...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!session) {
    if (req.method === 'GET') {
      return getTrainingCoursesPublic();
    }
  ...
  async function getTrainingCoursesPublic() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const findOptions = convertQueryToPrismaUtil(query, 'training_course');
    const countOptions = omit(findOptions, 'include');
    const [totalCount, data] = await prisma.$transaction([
      prisma.training_course.count(countOptions as unknown),
      prisma.training_course.findMany({
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function getTrainingCourses() {
    ...
    const limit = parseInt(_limit as string, 10) || 20;
    const offset = parseInt(_offset as string, 10) || 0;
    const response = await prisma.training_course
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'training_course'),
        take: limit,
        skip: offset,
        ...(order?.length && {
          orderBy: getOrderByOptions(order),
  ...
  async function createTrainingCourse() {
    ...
    const data = await prisma.training_course.create({
      data: body,
    });
