...
import { getNutritionistById } from 'apiSdk/nutritionists';
import { NutritionistInterface } from 'interfaces/nutritionist';

function NutritionistViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<NutritionistInterface>(
    () => (id ? `/nutritionists/${id}` : null),
    () =>
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('nutritionist', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/nutritionists/edit/${id}`} passHref legacyBehavior>
                    <Button
