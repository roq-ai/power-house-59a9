...
type ColumnType = ColumnDef<GymInterface, unknown>;

interface GymListPageProps {
...
export function GymListPage(props: GymListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getGyms({
        relations: ['user', 'gym_member.count', 'gym_trainer.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<GymInterface>>(
    () => `/gyms?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('gym_member', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'gym_member',
          header: 'Gym Member',
          accessorKey: 'gym_member',
          cell: ({ row: { original: record } }: any) => record?._count?.gym_member,
        }
      : null,
    hasAccess('gym_trainer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'gym_trainer',
          header: 'Gym Trainer',
          accessorKey: 'gym_trainer',
          cell: ({ row: { original: record } }: any) => record?._count?.gym_trainer,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/gyms/view/${record.id}`} passHref legacyBehavior>
                <Button
