...
import { getGymById, updateGymById } from 'apiSdk/gyms';
...
function GymEditPage() {
  ...
  const handleSubmit = async (values: GymInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      await imageUploaderRef.current.handleUpload();
      const updated = await updateGymById(id, values);
      mutate(updated);
      resetForm();
      router.push('/gyms');
    } catch (error: any) {
  ...
  const formik = useFormik<GymInterface>({
    initialValues: data,
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="gym" entityId={id} />
        </FormWrapper>
