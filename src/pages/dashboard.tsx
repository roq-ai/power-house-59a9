...
import { getTrainingCourses } from 'apiSdk/training-courses';
...
import { getGymMembers } from 'apiSdk/gym-members';
import { GymMemberInterface } from 'interfaces/gym-member';

import { TrainingCourseListPage as ShowCaseList } from 'pages/training-courses';

function DashboardPage() {
  const { session } = useSession();

  const trainingCourseStat = useSWR<PaginatedInterface<TrainingCourseInterface>>(
    () => `/training-courses`,
    () => getTrainingCourses({ limit: 1, offset: 0 }),
  );

  const nutritionistStat = useSWR<PaginatedInterface<NutritionistInterface>>(
    () => `/nutritionists`,
    () => getNutritionists({ limit: 1, offset: 0 }),
  );

  const gymMemberStat = useSWR<PaginatedInterface<GymMemberInterface>>(
    () => `/gym-members`,
    () => getGymMembers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Training Courses',
      result: trainingCourseStat?.data?.totalCount,
      isLoading: trainingCourseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Nutritionists',
      result: nutritionistStat?.data?.totalCount,
      isLoading: nutritionistStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Gym Members',
      result: gymMemberStat?.data?.totalCount,
      isLoading: gymMemberStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`training-courses`} passHref legacyBehavior>
                  <Button
