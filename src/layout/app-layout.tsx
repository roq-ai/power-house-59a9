...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['business-owner', 'gym-trainer', 'gym-member', 'nutritionist', 'guest-user'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<GymInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/gyms` : null),
    () => getGyms({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Gyms',
      path: isTenantUser ? `/gyms/view/${data?.[0]?.id}` : '/gyms',
      entity: 'gym',
